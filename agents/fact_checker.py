...
def fact_checker_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if status == "REJECT":
            ...
            print(f"--- [Fact-Checker Agent] Audit Failed: {status} ---")
        ...
        # Fallback if AI fails to follow JSON format
        if "REJECT" in response.upper():
            return {"errors": [f"REJECT: Unstructured failure - {response[:200]}"]}
        return {"errors": ["PASS"]} # Conservative pass if parsing fails but no reject keyword found
